...
class PPTXMorphGeneratorService:
    ...
    def _pct_to_inches(self, x_pct: float, y_pct: float, w_pct: float, h_pct: float) -> Tuple[Inches, Inches, Inches, Inches]:
        """Quy đổi tọa độ % màn hình sang đơn vị Inches của PowerPoint."""
        left = Inches((max(0.0, min(x_pct, 100.0)) / 100.0) * self.slide_width_in)
        top = Inches((max(0.0, min(y_pct, 100.0)) / 100.0) * self.slide_height_in)
        width = Inches((max(1.0, min(w_pct, 100.0)) / 100.0) * self.slide_width_in)
        height = Inches((max(1.0, min(h_pct, 100.0)) / 100.0) * self.slide_height_in)
    ...
    def _add_element_to_slide(self, slide, elem: SlideElement) -> None:
        ...
        if elem.type == "badge":
            ...
            p.font.bold = True
            p.font.size = Pt(elem.font_size or 10)
            p.font.color.rgb = text_rgb
            p.font.name = "Segoe UI"
        ...
        # Trường hợp 3: Card hoặc Metric Box hoặc Shape tiêu chuẩn
        ...
        if elem.border_color:
            shape.line.color.rgb = hex_to_rgb(elem.border_color)
        ...
        if elem.type == "metric":
            p_main.font.size = Pt(elem.font_size or 32)
            p_main.font.color.rgb = text_rgb
        else:
            p_main.font.size = Pt(elem.font_size or 15)
            p_main.font.color.rgb = text_rgb
